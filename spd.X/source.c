...
{   static unsigned char width, widbuf ; //PWMのON時間用変数
    ...
    if(TMR1IF){ //21Hz
        ...
        spdpl = 0;
            if(spdval >= 87){ //メータの仕様上針がπradで87km/h
                width = 49; //サーボモータはπrad以上動かないのでリミットを設ける
            }
                else if(spdval <= 10){
                   width = 239 - (spdval * 1.7);
                }
                else if (spdval == 0){
                   width = 239 ;
                }
                else{
                   width = 239 - (spdval * 2.26); //0-83の数値を49-239に変換
                }
